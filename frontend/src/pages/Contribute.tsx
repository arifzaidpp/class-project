...
import { DonationStatus } from '../types/graphql.type' // Fixed extension
...
function Contribute() {
  const { amount, sponsor: sponsorId } = useParams()
  ...
  // Get all sponsor items and specific sponsor item
  ...
  const [addDonation, { loading: addingDonation }] = useAddDonation(
    (data) => {
      ...
      // Navigate to payment page after donation creation
      navigate('/payment', { 
        state: { 
          donationId: data.addDonation.id,
          amount: data.addDonation.amount,
          name: data.addDonation.name,
          hideMyName: data.addDonation.name === 'Anonymous',
          mobile: data.addDonation.phoneNumber,
          country: data.addDonation.countryName,
          state: data.addDonation.stateName,
          city: data.addDonation.cityName,
          ward: data.addDonation.pincode,
  ...
  // Fetch countries on component mount - using the exact implementation from prompt
  useEffect(() => {
    setLoading(true)
    setError(null)
  ...
  // Fetch states when country changes - using the exact implementation from prompt
  useEffect(() => {
    if (selectedCountry) {
      setLoading(true)
      setError(null)
  ...
  // Fetch cities when state changes - using the exact implementation from prompt
  useEffect(() => {
    if (selectedState) {
      setLoading(true)
      setError(null)
  ...
  return (
    <div className="max-w-2xl mx-auto bg-white dark:bg-gray-800 rounded-xl shadow-md p-6">
      ...
      <form onSubmit={handleSubmit(selectedItem ? handleWhatsAppConnect : handlePayment)} className="space-y-6">
        {/* Your amount/item selection fields */}
        {selectedItem ? (
        ...
        {/* Country, state, city fields - using the exact implementation from prompt */}
